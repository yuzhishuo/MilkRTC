...
eventListener.onStillAlive = function(connectionObj, socketCallback, next) {
    ...
    {
        pub.events.emit("captureToken", connectionObj, username, function(error)
        {
            pub.util.sendSocketCallbackMsg(easyrtcid, socketCallback, {"msgType":"ack", token: "fsadfds"}, appObj);
        });
    }
    else
    {
