...
eventListener.onEasyrtcAuth = function(socket, easyrtcid, msg, socketCallback, callback) {
    ...
    // compatibility socket-io(uwp)
    if(_.isFunction(msg))
    {
        [msg, socketCallback] = [socketCallback, msg];
    }
    ...
    async.waterfall([
        ...
        function(isConnected, asyncCallback) {
            ...
            // if roomJoin is present in message, check the room names 这里仅仅检查room结构 // 后续 想把这个功能禁掉
        ...
        function(asyncCallback) {
            // fix
            if(!pub.getOption("appAnonymous"))
            {
                pub.events.emit("roomCreate", appObj, connectionObj, easyrtcid, null, function(err, roomObj) {
                    if(err) {
                        pub.util.logError("["+appName+"]["+easyrtcid+"] can't create room.", err);
                    }
                    pub.events.emit("roomJoin", connectionObj, easyrtcid, null, function(err, roomObj) {
                        
                        pub.events.emit("generatePeoples", connectionObj, easyrtcid, function(err)
                        {
                            if(err) pub.util.logError("["+appName+"]["+easyrtcid+"] can't generate Peoples.", err);
                            asyncCallback(null);
                        })
...
eventListener.onEasyrtcCmd = function(connectionObj, msg, socketCallback, next) {
    ...
    async.waterfall([
        ...
        function(asyncCallback) {
            ...
            switch(msg.msgType) {
                case "stillAlive":
                    pub.events.emit("stillAlive", connectionObj, socketCallback, next);
                    next(null);
                    break;
...
eventListener.onEmitCustomMsg = function(connectionObj, msg, next) {
    pub.util.logInfo("emit custom message");
    next(null);
}

eventListener.onStillAlive = function(connectionObj, socketCallback, next) {
    var appObj = connectionObj.getApp();
    var easyrtcid = connectionObj.getEasyrtcid();
    ...
    {
        pub.events.emit("captureToken", connectionObj, username, function(error, token)
        {
            if(error)
            {
                // 输出错误信息
            ...
            pub.util.sendSocketCallbackMsg(easyrtcid, socketCallback, { "msgType":"ack", token: token}, appObj);
        });
    }
    else
    {
...
eventListener.onCaptureToken = function(connectionObj, id, next)
{
    next(null, undefined);
}
            ...
            if (msg.targetEasyrtcid !== undefined) {
                ...
                async.waterfall([
                    ...
                    function(newTargetConnectionObj, asyncCallback) {
                        ...
                        if(newTargetConnectionObj === null)
                        {
                            if(!appObj.getOption("appAnonymous"))
                            {

                                pub.events.emit("emitCustomMsg", connectionObj, msg, function(error)
                                {
                                    if(error === null)
                                    {
                                        pub.util.sendSocketCallbackMessageAck(easyrtcid, socketCallback, msg.msgId , appObj);
                                    }
                                });
                                return;
                            }
                        ...
                        asyncCallback(newTargetConnectionObj);
                    },
                    function(newTargetConnectionObj, asyncCallback) {
...
eventListener.onEmitReturnToken = function(connectionObj, socketCallback, next) {
    ...
    async.waterfall([
        ...
        function(asyncCallback) {
            ...
            connectionObj.generateContactList(asyncCallback);
        },
        function(peopleList ,asyncCallback) {
            
            tokenMsg.msgData.Contact = peopleList;
...
eventListener.onGeneratePeoples = function(connectionObj, roomName, next)
{
    connectionObj.CreatePeopleList([]);
    next(null);
}
