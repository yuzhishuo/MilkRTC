...
/**
 * Get a list of user groups and friends based on user name
 *
 * @param  {function} pub.updataSubRoom
 */
pub.updataSubRoom = function(roomName)
{
    var friendsList = new Object();
    friendsList[roomName] ={};
    friendsList[roomName]["2321321"] = 
    {
        blacklist: false,
    }

    friendsList[roomName]["32131"] = {
        forbidden: false,
        admin: true,
        lord: false,
    }
...
pub.events.defaultListeners = {
    ...
    "generatePeoples":eventListener.onGeneratePeoples,
    "emitCustomMsg": eventListener.onEmitCustomMsg,
    "stillAlive": eventListener.onStillAlive,
    "captureToken": eventListener.onCaptureToken,
};
...
pub.util.sendSocketCallbackMessageAck = function(easyrtcid, socketCallback, messageid ,appObj) {
    return pub.util.sendSocketCallbackMsg(easyrtcid, socketCallback, { msgData: {msgId: messageid } ,"msgType":"ack"}, appObj);
};
...
pub.app = function(appName, callback) {
    ...
    appObj.connection = function(easyrtcid, callback) {
        ...
        connectionObj.CreatePeopleList = function (peoples) {
           
            if( peoples === null || peoples === undefined)
            {
                var peoplelist = e.app[appName].connection[easyrtcid].peoplelist = [];
                return;
            }
            if(!_.isArray(peoples))
            {
                pub.util.logWarning("["+easyrtcid+"]" +  "peoples must is array");
            }

            for(var i = 0; i < peoples.length; i++)
            {
                e.app[appName].connection[easyrtcid].peoplelist[
                    peoples[i].user_id === undefined ? peoples[i] : peoples[i].user_id
                ] = peoples[i];
            }
        }

        connectionObj.GetPeopleList = function () {
            
            return e.app[appName].connection[easyrtcid].peoplelist;
        }
        /**
            ...
            async.waterfall([
                ...
                function(roomNames, asyncCallback) {
                    ...
                    async.each(roomNames,
                        ...
                        function(err) {
                            appObj.deleteRoom(easyrtcid,asyncCallback);
                        }
                    );
                },
                function(asyncCallback) {
 ...
 * Returns connection level field value for a given field name. If the field is not set, it will return a null field value.  This is a synchronous function, thus may not be available in custom cases where state is not kept in memory.
        ...
        connectionObj.generateContactList = function(callback)
        {
            var peoplelist = e.app[appName].connection[easyrtcid].peoplelist;

            callback(null ,peoplelist)
        }
                ...
                if (e.app[appName].connection[easyrtcid].room.hasOwnProperty(currentRoomName)) {
                    ...
                    for (var currentEasyrtcid in connectionRoom.toRoom.clientList) {
                        if (connectionRoom.toRoom.clientList.hasOwnProperty(currentEasyrtcid)) {
                            ...
                            // TO Do: 
                            if(currentRoomName === easyrtcid)
                            {
                                var Peoples = connectionObj.GetPeopleList();
                                for(var people in Peoples)
                                {
                                    roomData[currentRoomName].clientList[Peoples[people].id] = {
                                        "easyrtcid": Peoples[people].id,
                                        "roomJoinTime": 0,
                                        "info": Peoples[people]
                                    };
                                }
                ...
                if (e.app[appName].connection[easyrtcid].room.hasOwnProperty(currentRoomName)) {
                    ...
                    if (isLeavingRoom) {
                        ...
                        if(currentRoomName === easyrtcid)
                        {
                            ...
                            for(var people in peoples)
                            {
                                roomDataDelta[currentRoomName].clientListDelta.updateClient[people.toString()] = {
                                    "apiField" :{},
                                    "easyrtcid": people,
                                    "roomJoinTime": Date.now().toString(),
                                    "presence": {}
                                };
                            }
                        }
            ...
            for (var currentRoomName in e.app[appName].room) {
                if (e.app[appName].room.hasOwnProperty(currentRoomName)) {
                    roomList[currentRoomName] = {
                        "roomName": currentRoomName,
                        "numberClients": _.size(e.app[appName].room[currentRoomName].clientList) + _.size(currentRoomName === easyrtcid? this.GetPeopleList(): [])
                    };
                }
            }
        ...
        connectionObj.joinRoom = function(roomName, callback)
        {
            ...
            var createConnectionRoom = function(roomName, appRoomObj, callback) {
                ...
                e.app[appName].room[roomName].subroom = pub.updataSubRoom(roomName);
        ...
        connectionObj.room = function(roomName, callback) {
            ...
            connectionRoomObj.generateRoomDataDelta = function(isLeavingRoom, callback) {
                ...
                if (isLeavingRoom) {
                    ...
                    if( this.getRoomName() == easyrtcid)
                    {
                        var peoples = connectionObj.GetPeopleList();
    
                        for(var people in peoples)
                        {
                            roomDataDelta.clientListDelta.updateClient[people] = {
                                "easyrtcid": people,
                                "roomJoinTime": 0,
                                "username": people,
                                "presence": {
        ...
        connectionObj.removeConnection = function(next) {
            if (e.app[appName] && _.isObject(e.app[appName].connection) && e.app[appName].connection[easyrtcid]) {
                ...
                // Remove connection create
                delete e.app[appName].room[easyrtcid]
    ...
    appObj.createRoom = function(roomName, options, callback) {
        ...
        e.app[appName].room[roomName] = {
            ...
            clientList: {},
            field: {},
            // subroom[userid]["2321321"] = 
            // {
            //     blacklist: false,
            // }

            // subroom[roomid]["32131"] = {
            //     forbidden: false,
            //     admin: true,
            //     lord: false,
            // }
        ...
        // e.app[appName].room[roomName].subroom = 
    ...
    appObj.isRoomSync = function(roomName) {
        return ((e.app[appName] && 
            e.app[appName].room[roomName] &&
            !e.app[appName].room[roomName].deleted) ? true : false);
    };
    ...
    appObj.room = function(roomName, callback) {
        ...
        roomObj.getSubrooms= function(roomName)
        {
            var subroom = {};
           return e.app[appName].room[roomName].subroom[id]
        }
